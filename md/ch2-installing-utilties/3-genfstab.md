# genfstab from Arch Install Scripts
A tool to generate an fstab file from a mounted set of directories

# Install the genfstab script, as the root user
```bash
cat >> /usr/bin/genfstab << 'EoF'
#!/bin/bash
